...
//getActivities('divapprreports','tableapprreports','commentreportsapprreports','getapprouverops')
	$.ajax({
		method: "POST",
		url: "/adminPortal/api/" + partURI + "/" + dynamicPart,
		contentType: "application/json",
		dataType: "json", 
		async: true
	}
...
let getChangeStatusReports01 = () => {
	...
	$.ajax({
		...
		$(".commentChangeStatusReport01").append("<p class ='alert alert-danger  p-0'  style ='padding:3px'> Erreur  chargement du rapport </p>");
	}).always(function() {
	});
}
